...
const SYSTEM_PROMPT = `You are an edge-case hunter. Given a plan document or codebase, identify boundary conditions, failure modes, and what-if scenarios.\n\nFocus on: Boundary conditions, Race conditions, Timezone pitfalls, Off-by-one errors, Unhandled failures, Input validation gaps, Resource exhaustion, Cascading failures.\n\nRESPOND IN EXACTLY THIS JSON FORMAT (no markdown, no wrapping):\n{\"findings\": [{\"severity\": \"critical\" | \"warning\" | \"info\", \"title\": \"short title\", \"evidence\": \"what edge case\", \"file\": \"path/to/file\", \"line\": 42, \"recommendation\": \"how to handle\"}], \"score\": <0-100>, \"summary\": \"one-line summary\"}\n\nRules: critical = crash/data loss. warning = degraded behavior. info = observation. score: 100=robust, 75=minor gaps, 50=several unhandled, 25=fragile, 0=will crash.`;
...
function buildUserPrompt(input: AgentInput, fileContents: string): string {
  ...
  return `${header}\n\nAnalyze the following content for edge cases, boundary conditions, and failure modes.\n\n---\n${fileContents.slice(0, 12000)}\n---\n\nProvide your findings in the exact JSON format specified.`;
}
...
export function createEdgeAgent(modelFn: ModelFn): VouchItAgent {
  return {
    id: 'edge', displayName: 'Edge-Case Hunter',
    async audit(input: AgentInput): Promise<AgentResult> {
      ...
      try {
        ...
        return { findings: [{ id: 'EDG-000', severity: 'info', agent: 'edge', title: 'Edge-case agent returned unparseable output', evidence: text.slice(0, 500), recommendation: 'Retry with a different model or check model availability' }], score: 0, confidence: 'low' };
      }
      return { findings: mapFindings(parsed.findings, 'edge'), score: parsed.score, confidence: deriveConfidence(parsed.score) };
    },
  };
}